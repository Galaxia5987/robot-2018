...
public class CommandGroup_ScaleFull extends CommandGroup {
	...
	public CommandGroup_ScaleFull(String gameData, char robotStartingLocation) { // Add
		...
		addSequential(new DriveDistanceCM(712.6224)); // drive until the
														// platformzone line
		if (gameData.charAt(1) == robotStartingLocation) { // if the scale is on
															// the same side as
															// the robot
			addParallel(new Lift(2)); // lift the arm while it drives to the
										// bump
			addSequential(new DriveUntilTrigger());// drives until the bump is
													// hit

			addSequential(new TurnAngle(90 * angleMultiplier)); // turn towards
																// the switch
			// driveabit
			addSequential(new DropBox()); // release the box
			addSequential(new TurnAngle(90 * angleMultiplier)); // turn again
			addParallel(new Lift(0)); // lower the arm while going back to the
										// switch
			addSequential(new DriveDistanceCM(88.9)); // drive back towards the
														// first box
			addSequential(new TurnAngle(-45 * angleMultiplier)); // turn towards
																	// it
		} else {
			addSequential(new TurnAngle(90 * angleMultiplier)); // turn to the
																// inwards of
																// the platform
