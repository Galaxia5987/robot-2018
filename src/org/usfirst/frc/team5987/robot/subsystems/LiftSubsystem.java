...
public class LiftSubsystem extends Subsystem {
	...
	/**
	 * Decreasing rate for the output (substructs this from the setpoint every
	 * iteration)
	 */
	...
	NetworkTableEntry ntBalance = LiftTable.getEntry("Zero When Tipping");
	...
	public LiftSubsystem() {
		...
		pid = new MiniPID(ntTopKp.getDouble(topPID[0]), ntTopKi.getDouble(topPID[1]), ntTopKd.getDouble(topPID[2]));
		// Add the NetworkTable entries if they don't exist
		ntBalance.setBoolean(ntBalance.getBoolean(true));
	...
	public double _update(double height, boolean bottomHallEffect, boolean isEnabled) {
		...
		// Lower the Lift if the robot is tipping
		if (Robot.driveSubsystem.getState() == DriveSubsystem.DriveStates.BALANCING 
				&& state == States.RUNNING
				&& ntBalance.getBoolean(true))
